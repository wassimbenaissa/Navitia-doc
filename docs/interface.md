...
* containing some [disruptions](/objects/real-time-and-disruption-objects#disruption) objects (**"type": "disruption"**)
...
We also allow Navitia to add values to enum, so be prepared to that. For example [section's](/api/journeys#section) type of journeys are regularly evolving.
...
We recommend to use as much as possible a previous call to **[Places](/api/autocomplete-places)** and **[PT_objects](/api/autocomplete-pt-objects)**.
